import { Shuffle, Sparkles, Trophy, Zap, Rocket, Flame, Star, Ghost, Gem, PartyPopper, Crown, Grid3X3, Users, UserMinus, Tv } from 'lucide-react';

export default function PlayerView({ 
    grid, marked, toggleTile, handleShuffleClick, bingoCount, gameMode, currentDraw, participants, isHost, session, sessionId, myUserId, supabase, showShuffleConfirm, setShowShuffleConfirm, confirmShuffle 
}) {
    ...
    const getBranding = (rowCount, isFull) => {
        ...
        if (gameMode === 'full') {
            ...
            return { title: "SPELEND..", icon: <Grid3X3 size={16} className="opacity-50"/>, lobbyClass: "bg-white border-gray-50 text-gray-700" };
        }
        const level = Math.min(rowCount, 12);
        const branding = titles[level] || titles[0];
    ...
    return (
        <div className="flex flex-col lg:flex-row items-start justify-center gap-8 w-full max-w-7xl mx-auto">
            ...
            {/* RECHTER KOLOM: LOBBY */}
            {participants.length > 0 && gameMode !== 'hall' && (
                <div className="w-full lg:w-96 shrink-0 bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-6 shadow-2xl border border-white/50 mb-12 lg:sticky lg:top-32">
                    <div className="flex items-center justify-between mb-6 pl-2"><h3 className="text-xl font-black text-gray-900 flex items-center gap-3 italic uppercase"><Users className="text-orange-500" size={24} /> Lobby <span className="bg-orange-100 text-orange-600 px-2 py-0.5 rounded-lg text-sm not-italic ml-1">{participants.length} / {session.max_players || 50}</span></h3></div>
                    <div className="space-y-3 max-h-[60vh] overflow-y-auto p-4 -mx-4 custom-scrollbar">
                        {participants.map((p) => {
                            ...
                            // Check of de speler 'iets' heeft bereikt om de stijl te activeren
