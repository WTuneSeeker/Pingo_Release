...
/**
 * BINGO CONQUEST: ONLINE MULTIPLAYER (FIXED TIMER)
 * 1v1 Real-time Arena. 
 */
...
export default function RankedArena() {
  ...
  const joinQueue = () => {
    setView('searching');
    setStatusMsg('Zoeken naar tegenstander...');
    ...
    channel
      ...
      .on('broadcast', { event: 'game_start' }, ({ payload }) => {
        // Alleen de gast ontvangt dit, de host start lokaal
      ...
      .on('broadcast', { event: 'tile_claimed' }, ({ payload }) => {
        if (payload.playerId !== user.id) syncRivalClaim(payload.idx, payload.points);
      })
      .on('broadcast', { event: 'bomb_detonated' }, ({ payload }) => {
        if (payload.attackerId !== user.id) syncBombHit(payload.targetIdx);
      })
      .on('broadcast', { event: 'game_over' }, () => {
        // Forceer einde match als de host zegt dat het klaar is
        finishGame();
      })
  ...
  const startMultiplayerMatch = () => {
    // Host Logic: Genereer bord en stuur startsignaal
    if (isHostRef.current && !gameActiveRef.current) {
  ...
  const beginMatch = () => {
    ...
    setLogs([{ msg: "NEURAL LINK ESTABLISHED. FIGHT!", type: 'sys' }]);
    setView('playing');
    gameActiveRef.current = true;
    
    // Start Timer voor IEDEREEN
    initLocalTimer();

    // Host start de getallen-generator
    if (isHostRef.current) {
      initHostEngines();
    }
  };

  // Timer die lokaal draait voor UI updates
  const initLocalTimer = () => {
    setTimeLeft(180); // Reset naar 3 minuten
    if (timerRef.current) clearInterval(timerRef.current);
    ...
    timerRef.current = setInterval(() => {
        ...
        setTimeLeft(prev => {
            if (prev <= 1) {
                // Als Host de 0 bereikt, stuur officieel eindsignaal
                if (isHostRef.current) {
  ...
  const initHostEngines = () => {
    ...
    }, 4000); // Iets rustiger tempo (4s) voor strategie
  };
  ...
  const syncRivalClaim = (idx, pts) => {
    ...
    scoresRef.current = newScores;
    setScores(newScores);
    addLog(`Rivaal claimt ${grid[idx]}!`, 'err');
  };
  ...
  const localBombDetonate = (idx, attackerName) => {
    ...
    addLog(`${attackerName} gebruikte een BOM!`, 'sys');
  };
  ...
  const syncBombHit = (idx) => {
    ...
    claimsRef.current = nc;
    setClaims(nc);
    addLog("OEPSS! Je gebied is geraakt door een BOM!", 'err');
  };
  ...
  const detectForts = (idx, pid, currentClaims) => {
    const quadrants = [[idx, idx+1, idx+10, idx+11], [idx-1, idx, idx+9, idx+10], [idx-10, idx-9, idx, idx+1], [idx-11, idx-10, idx-1, idx]];
    let found = false;
  ...
  if (view === 'loading') return <div className="h-screen bg-black flex items-center justify-center text-red-500 font-black animate-pulse">CONNECTING...</div>;
  ...
  if (view === 'lobby' || view === 'searching') {
    return (
      <div className="h-screen bg-[#050505] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-red-600/5 blur-[150px] rounded-full"></div>
        <div className="text-center space-y-10 max-w-xl w-full z-10 animate-in fade-in duration-700">
            <div className="bg-white/5 p-12 rounded-[4rem] border border-white/10 inline-block shadow-2xl relative">
                ...
                <div className="absolute -top-4 -right-4 bg-red-600 text-[10px] font-black px-4 py-2 rounded-full shadow-xl">ONLINE</div>
            </div>
            <div>
                <h1 className="text-8xl font-black italic uppercase tracking-tighter">CON<span className="text-red-600">QUEST</span></h1>
                <p className="text-gray-500 font-bold uppercase tracking-[0.4em] text-xs mt-2">1v1 Ranked Multiplayer</p>
            </div>
            {view === 'searching' ? (
                <div className="bg-white/5 p-12 rounded-[3.5rem] border border-white/10 backdrop-blur-xl space-y-6">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 font-bold uppercase mt-2">Waiting for player...</p>
                    </div>
                    <button onClick={() => window.location.reload()} className="text-gray-600 font-black uppercase text-[10px] tracking-widest border-b border-gray-800 pb-1">Cancel</button>
                </div>
            ) : (
                <div className="flex flex-col gap-4 px-16">
                    <button onClick={joinQueue} className="bg-red-600 hover:bg-red-500 text-white py-8 rounded-[3rem] font-black text-3xl uppercase tracking-widest shadow-[0_25px_60px_rgba(220,38,38,0.3)] transition-all hover:scale-[1.02]">FIND MATCH</button>
                    <button onClick={() => navigate('/dashboard')} className="text-gray-600 font-black uppercase text-xs hover:text-white transition-colors">Return</button>
                </div>
            )}
        </div>
      </div>
    ...
    <div className="h-screen bg-[#020202] text-white flex flex-col lg:flex-row overflow-hidden font-sans relative select-none">
        ...
        <div className="w-full lg:w-[400px] bg-[#080808] border-r border-white/5 p-10 flex flex-col gap-10 z-30 shadow-2xl relative">
            <div className="flex items-center gap-6 p-6 bg-white/5 rounded-[3rem] border border-white/10">
                ...
                <div><p className="text-[10px] font-black text-red-500 uppercase tracking-widest mb-1">Player 1 (You)</p><p className="font-black uppercase italic text-2xl truncate">{user?.username}</p></div>
            </div>

            <div className="space-y-8 flex-1">
                <div className="flex justify-between items-center px-2">
                    <p className="text-[11px] font-black text-gray-500 uppercase tracking-[0.4em] flex items-center gap-3"><Globe size={18}/> Match Stats</p>
                    <div className="flex items-center gap-2"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><span className="text-[8px] font-black text-green-500 uppercase">Live</span></div>
                </div>

                <div className="space-y-10">
                    <div className="space-y-3">
                        <div className="flex justify-between items-end"><span className="text-xs font-black uppercase text-white">Jij</span><span className="text-red-500 font-black text-2xl">{scores.me}</span></div>
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between items-end"><span className="text-xs font-black uppercase text-gray-600">Rivaal</span><span className="text-gray-400 font-black text-2xl">{scores.rival}</span></div>
            ...
            <div className="p-8 bg-white/5 rounded-[3.5rem] border border-white/10 text-center relative overflow-hidden">
                ...
                <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest mt-2">Time Remaining</p>
            </div>
        </div>
        ...
        <div className="flex-1 p-8 lg:p-12 flex flex-col items-center justify-between relative z-20">
            <div className="w-full max-w-xl text-center space-y-4 pt-4">
                <div className="flex items-center justify-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-red-600 uppercase tracking-[0.8em]">Neural Stream</span>
                </div>
                <div className={`bg-white text-[#050505] py-10 rounded-[4rem] font-black text-7xl lg:text-9xl uppercase italic shadow-[0_0_120px_rgba(255,255,255,0.15)] border-[12px] border-white/5 transition-all duration-500 ${currentEvent.type === 'bomb' ? 'bg-red-600 text-white scale-110 rotate-1' : ''}`}>
            ...
            <div className={`grid grid-cols-10 gap-1.5 w-full max-w-[720px] aspect-square my-8 transition-all duration-700 relative ${isFrozen ? 'blur-2xl scale-95 opacity-50' : ''}`}>
                {grid.map((num, i) => {
                    ...
                    return (
                        <div key={i} onClick={() => handleTileClick(i)} className={`aspect-square rounded-lg border-2 flex items-center justify-center text-center cursor-pointer transition-all duration-300 relative overflow-hidden ${owner === user.id ? 'bg-red-600 border-red-400 shadow-[inset_0_0_25px_rgba(255,255,255,0.3)] z-10' : owner === 'rival' ? 'bg-blue-600 border-blue-400 opacity-90' : isTarget ? 'bg-white/20 border-white animate-pulse scale-105 z-20 shadow-2xl' : 'bg-white/5 border-white/5 hover:border-white/20'} ${isJackpot && !owner ? 'border-yellow-400 border-[3px] shadow-[0_0_20px_rgba(250,204,21,0.4)]' : ''}`}>
                            <span className={`font-black italic transition-all ${owner ? 'text-white text-[10px] lg:text-sm scale-110' : 'text-gray-700 text-[9px] lg:text-xs'}`}>{num}</span>
                ...
                <div className="absolute inset-0 bg-red-950/50 backdrop-blur-3xl z-[100] flex items-center justify-center animate-in fade-in duration-300">
                    <div className="bg-red-600 text-white px-16 py-8 rounded-[4rem] font-black uppercase italic animate-bounce shadow-2xl border-8 border-red-400 flex flex-col items-center gap-6">
                        ...
                        <div className="text-center">
                            <p className="text-5xl tracking-tighter">PENALTY</p>
        ...
        <div className="w-full lg:w-[450px] bg-[#080808] border-l border-white/5 p-10 flex flex-col gap-10 z-30 shadow-2xl relative">
            <div className="flex items-center justify-between text-[11px] font-black text-gray-500 uppercase tracking-[0.4em] mb-2 px-2">
                <div className="flex items-center gap-3"><Terminal size={18}/> Combat Feed</div>
                <div className="flex items-center gap-2 text-green-500"><Wifi size={12}/> ONLINE</div>
            </div>
            ...
            <div className="flex-1 bg-black/60 rounded-[3.5rem] border border-white/5 p-8 overflow-hidden relative shadow-inner">
                ...
                <div className="flex flex-col-reverse gap-6 relative z-0 h-full overflow-hidden">
                    {logs.map(log => (
                        <div key={log.id} className={`text-xs font-bold uppercase italic leading-relaxed animate-in slide-in-from-right-10 duration-500 flex items-start gap-4 ${log.type === 'success' ? 'text-red-400' : log.type === 'ai' ? 'text-blue-500' : log.type === 'err' ? 'text-amber-500' : 'text-gray-600'}`}>
                            <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${log.type === 'success' ? 'bg-red-500' : log.type === 'ai' ? 'bg-blue-500' : 'bg-gray-700'}`}></div>
        ...
        {view === 'results' && (
            <div className="fixed inset-0 z-[1000] bg-black/98 backdrop-blur-3xl flex items-center justify-center p-6 animate-in fade-in duration-1000">
                <div className="bg-white/5 p-20 rounded-[5rem] border-2 border-white/10 max-w-2xl w-full text-center shadow-2xl relative overflow-hidden">
                    ...
                    <h2 className="text-8xl font-black uppercase italic mb-10 tracking-tighter leading-none relative z-10">{scores.me > scores.rival ? 'VICTORY' : 'DEFEAT'}</h2>
                    <div className="bg-white/5 p-10 rounded-[3.5rem] border border-white/10 mb-12">
                        <p className="text-[10px] font-black text-gray-500 uppercase mb-2">Final Score</p>
                        <p className="text-7xl font-black text-red-600 italic leading-none">{scores.me}</p>
                    </div>
                    <button onClick={() => navigate('/dashboard')} className="w-full py-10 bg-white text-[#050505] rounded-[3.5rem] font-black text-3xl uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-xl relative z-10">RETURN TO HUB</button>
                </div>
            </div>
        )}
    </div>
